...
@test "backup-all.sh requiere .env" {
	run env PROJECT_ROOT="$TEST_TMP_DIR" \
		bash "$TEST_SCRIPTS_DIR/commands/backup-all.sh"

	# Sin .env puede fallar o ejecutarse sin servicios
	[[ $status -ge 0 ]]
}
...
@test "backup-all.sh detecta servicios desde .env" {
	...
	[[ $status -ge 0 ]]
	assert_contains "$output" "postgres\|mongo\|servicios\|Resumen\|backup" "Debería detectar servicios desde .env"
}
...
@test "backup-all.sh acepta opción --skip-missing" {
	...
	run env PROJECT_ROOT="$TEST_TMP_DIR" \
		bash "$TEST_SCRIPTS_DIR/commands/backup-all.sh" --skip-missing
...
@test "backup-all.sh usa variable de entorno SKIP_MISSING" {
	...
	run env SKIP_MISSING=true PROJECT_ROOT="$TEST_TMP_DIR" \
		bash "$TEST_SCRIPTS_DIR/commands/backup-all.sh"
...
@test "backup-all.sh maneja servicios no encontrados con --skip-missing" {
	...
	[[ $status -ge 0 ]]
	assert_contains "$output" "omitido\|no encontrado\|Resumen\|backup" "Debería indicar servicios omitidos"
}
...
@test "backup-all.sh muestra resumen de operaciones" {
	...
	assert_contains "$output" "Resumen\|exitosos\|fallidos\|omitidos\|backup" "Debería mostrar resumen"
}
